...
pub enum FrozenAction {
    ...
    /// activate it; when active, cycle the shape (Circle → Diamond →
    /// RoundedRect) then turn the layer off on the last shape.
    ToggleMode(ModeKind),
...
mod tests {
    ...
    #[test]
    fn plan_is_spotlight_capture_reset_copy_cancel() {
    ...
    fn plan_reads_settings_not_hardcoded_keys() {
        ...
        assert_eq!(planned(&plan, h.reset_zoom), vec![FrozenAction::ResetZoom]);
    }
    ...
    fn default_settings_plan_has_all_five_registrations() {
        ...
        // wheel chord, so no `ToggleMode(ModeKind::Zoom)` is registered. The
        // shape-cycle hotkey was removed: the spotlight key `S` now cycles
        // shapes and turns off on the last shape.
